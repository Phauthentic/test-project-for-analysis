...
/**
 * Converts PHPStan --error-format=json output to GitHub Checks annotations JSON
 * (array of objects) accepted by Domain Atlas github-actions importer.
 *
 * Usage: php convert-phpstan-json-to-github-annotations.php <phpstan.json> [projectRoot]
 */

if ($argc < 2) {
    fwrite(STDERR, "Usage: php convert-phpstan-json-to-github-annotations.php <phpstan.json> [projectRoot]\n");
    exit(1);
}
...
foreach ($files as $absolutePath => $fileData) {
    ...
    foreach ($messages as $msg) {
        ...
        $text = isset($msg['message']) ? (string)$msg['message'] : 'PHPStan issue';
        $line = isset($msg['line']) ? (int)$msg['line'] : 1;
        $identifier = isset($msg['identifier']) ? (string)$msg['identifier'] : 'phpstan';
